...
const T = {
  en: {
    ...
    live_real: "LIVE data", live_sim: "demo data",
...
const COINS = [
  ...
  { sym: "XAU", name: "Gold (oz)", base: 4185, vol: 0.006, sector: "commodity", cat: "commodity" },
  { sym: "XAG", name: "Silver (oz)", base: 55, vol: 0.012, sector: "commodity", cat: "commodity" },
  { sym: "WTI", name: "Crude Oil WTI (bbl)", base: 74, vol: 0.015, sector: "commodity", cat: "commodity" },
  { sym: "BRENT", name: "Brent Crude (bbl)", base: 78, vol: 0.014, sector: "commodity", cat: "commodity" },
  { sym: "XPT", name: "Platinum (oz)", base: 1080, vol: 0.01, sector: "commodity", cat: "commodity" },
  { sym: "XPD", name: "Palladium (oz)", base: 1150, vol: 0.014, sector: "commodity", cat: "commodity" },
  { sym: "HG", name: "Copper (lb)", base: 4.85, vol: 0.013, sector: "commodity", cat: "commodity" },
];
...
// --- real data (MEXC public API via /api/mexc proxy) ---
const USE_LIVE = true; // set false to fall back to pure simulation
const MEXC_INTERVAL = { "1m": "1m", "3m": "5m", "5m": "5m", "15m": "15m", "30m": "30m", "1H": "60m", "2H": "60m", "4H": "4h", "6H": "4h", "12H": "1d", "1D": "1d", "1W": "1W" };
function mexcPair(sym) { return sym + "USDT"; }
// commodities with real spot prices via gold-api.com (through proxy)
const LIVE_METALS = { XAU: "XAU", XAG: "XAG", XPT: "XPT", XPD: "XPD", HG: "HG" };
// crude oil via API Ninjas (through proxy): WTI + Brent
const LIVE_OIL = { WTI: "crude_oil", BRENT: "brent_crude_oil" };
async function fetchMetalPrice(sym) {
  const code = LIVE_METALS[sym];
  if (!code) return null;
  const r = await fetch("/api/mexc?path=metal&symbol=" + code);
  if (!r.ok) throw new Error("metal " + r.status);
  const j = await r.json();
  const px = parseFloat(j.price);
  return isFinite(px) ? px : null;
}
async function fetchOilPrice(sym) {
  const name = LIVE_OIL[sym];
  if (!name) return null;
  const r = await fetch("/api/mexc?path=oil&symbol=" + name);
  if (!r.ok) throw new Error("oil " + r.status);
  const j = await r.json();
  const px = parseFloat(j.price);
  return isFinite(px) ? px : null;
}
async function fetchMexcKlines(sym, interval, limit = 120) {
  const mi = MEXC_INTERVAL[interval] || "15m";
  // (kept below) original klines logic continues
  return _fetchMexcKlinesImpl(sym, mi, limit);
}
// Calls our /api/ai proxy with automatic retry + timeout so transient Gemini
// hiccups don't force the user to tap several times.
async function callAI(prompt, { retries = 2, timeoutMs = 30000 } = {}) {
  let lastErr = "";
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (data.text) return { text: data.text, error: "" };
      lastErr = data.error || "EMPTY";
      // transient errors worth retrying: timeouts, 429 rate limit, 5xx, empty
      const retryable = /429|50\d|EMPTY|UNAVAILABLE|overloaded|timeout/i.test(lastErr);
      if (!retryable) return { text: "", error: lastErr };
    } catch (e) {
      lastErr = String(e); // network error or abort — always retry
    }
...
async function _fetchMexcKlinesImpl(sym, mi, limit = 120) {
  const url = "/api/mexc?path=klines&symbol=" + mexcPair(sym) + "&interval=" + mi + "&limit=" + limit;
  ...
  return raw.map((k, i) => {
    ...
    return { t: i, price: close, open, close, high, low, volume, up: close >= open };
  });
}
...
/* ================================== app =================================== */
// persist a piece of state to the browser's localStorage so it survives reloads.
// (works in the deployed app; safely no-ops if storage is unavailable)
function usePersist(key, initial) {
  ...
  useEffect(() => {
    try { if (typeof localStorage !== "undefined") localStorage.setItem(key, JSON.stringify(val)); } catch (e) { /* ignore quota/private-mode */ }
  }, [key, val]);
  return [val, setVal];
}
export default function CryptoTerminal() {
  const isMobile = useIsMobile();
  const [lang, setLang] = usePersist("abyss_lang", "zh");
  const t = T[lang];
  const [tab, setTab] = useState("dash");
  const [active, setActive] = useState("BTC");
  const [watchSyms, setWatchSyms] = usePersist("abyss_watch", DEFAULT_SYMS);
  const [tf, setTf] = useState("15m");
  const [exchange, setExchange] = useState("Binance");
  const [positions, setPositions] = usePersist("abyss_positions", [
    { id: 1, sym: "BTC", qty: 0.15, entry: 61200 },
    { id: 2, sym: "ETH", qty: 2.5, entry: 3050 },
    { id: 3, sym: "SOL", qty: 40, entry: 145 },
  ]);
  const [tick, setTick] = useState(0);
  const [journal, setJournal] = usePersist("abyss_journal", [
  ...
  const [alerts, setAlerts] = usePersist("abyss_alerts", [
  ...
  useEffect(() => {
    ...
    COINS.forEach((c) => {
      ...
      if (USE_LIVE && c.cat !== "commodity") return; // live data handles cryptos; only simulate commodities
  ...
  // --- LIVE: fetch real MEXC klines for the active coin (on coin/timeframe change + every 15s) ---
  const [liveOK, setLiveOK] = useState(false);
  useEffect(() => {
    if (!USE_LIVE) return;
    const c = COINS.find((x) => x.sym === active);
    if (!c) { setLiveOK(false); return; }
    let cancelled = false;

    // commodities: metals (gold-api) and oil (API Ninjas) have real spot prices; rest stay simulated
    if (c.cat === "commodity") {
      const isMetal = !!LIVE_METALS[active];
      const isOil = !!LIVE_OIL[active];
      if (!isMetal && !isOil) { setLiveOK(false); return; }
      const loadCommodity = async () => {
        try {
          const px = isMetal ? await fetchMetalPrice(active) : await fetchOilPrice(active);
          if (cancelled || !px) return;
          // anchor the simulated candle series to the real spot price
          const s = dataRef.current[active];
          const last = s[s.length - 1];
          const drift = px / last.price;
          // rescale whole series so it ends exactly at the real price (keeps chart shape)
          dataRef.current[active] = s.map((d) => ({
            ...d, price: d.price * drift, open: d.open * drift, close: d.close * drift,
            high: d.high * drift, low: d.low * drift,
          }));
          setLiveOK(true);
          setTick((x) => x + 1);
        } catch (e) { if (!cancelled) setLiveOK(false); }
      };
      loadCommodity();
      const id = setInterval(loadCommodity, isOil ? 60000 : 30000);
      return () => { cancelled = true; clearInterval(id); };
    }

    const load = async () => {
      try {
        const candles = await fetchMexcKlines(active, tf, 120);
        if (cancelled || candles.length === 0) throw new Error("empty");
        dataRef.current[active] = candles.map((d, i) => ({ ...d, t: i }));
        setLiveOK(true);
        setTick((x) => x + 1);
      } catch (e) {
        // klines failed (e.g. blocked) — try at least the real spot price and anchor the sim series to it
        try {
          const r = await fetch("/api/mexc?path=ticker/price&symbol=" + active + "USDT");
          if (!r.ok) throw new Error("price " + r.status);
          const j = await r.json();
          const px = parseFloat(j.price);
          if (cancelled || !isFinite(px)) throw new Error("no price");
          const s = dataRef.current[active];
          const drift = px / s[s.length - 1].price;
          dataRef.current[active] = s.map((d) => ({
            ...d, price: d.price * drift, open: d.open * drift, close: d.close * drift,
            high: d.high * drift, low: d.low * drift,
          }));
          ...
          if (!cancelled) setLiveOK(false); // keep simulated data on total failure
        }
      }
    };
    load();
  ...
  // --- LIVE: refresh watchlist prices every 20s via 24hr ticker ---
  useEffect(() => {
    if (!USE_LIVE) return;
    let cancelled = false;
    const load = async () => {
      const all = [...new Set([active, ...watchSyms])];
      const cryptos = all.filter((sym) => { const c = COINS.find((x) => x.sym === sym); return c && c.cat !== "commodity"; });
      const metals = all.filter((sym) => LIVE_METALS[sym] && sym !== active);
      const oils = all.filter((sym) => LIVE_OIL[sym] && sym !== active);
      await Promise.all([
        ...cryptos.map(async (sym) => {
          try {
            const r = await fetch("/api/mexc?path=ticker/price&symbol=" + sym + "USDT");
            if (!r.ok) return;
            const j = await r.json();
            const px = parseFloat(j.price);
            if (!isFinite(px)) return;
            const s2 = dataRef.current[sym];
            if (s2 && sym !== active) {
              const last = s2[s2.length - 1];
              last.price = px; last.close = px;
            }
          } catch (e) { /* ignore, keep sim */ }
        }),
        ...metals.map(async (sym) => {
          try {
            const px = await fetchMetalPrice(sym);
            if (!px) return;
            const s2 = dataRef.current[sym];
            if (s2) { const last = s2[s2.length - 1]; last.price = px; last.close = px; }
          } catch (e) { /* ignore */ }
        }),
        ...oils.map(async (sym) => {
          try {
            const px = await fetchOilPrice(sym);
            if (!px) return;
            const s2 = dataRef.current[sym];
            if (s2) { const last = s2[s2.length - 1]; last.price = px; last.close = px; }
          } catch (e) { /* ignore */ }
  ...
  // when showing real live data, don't apply the simulated exchange spread — show the true price
  const exMul = liveOK ? 1 : (1 + (EX_OFFSET[exchange] || 0));
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: "100vh", fontFamily: "ui-sans-serif, system-ui" }}>
      ...
      <div style={{ padding: isMobile ? "10px 12px" : "12px 18px", borderBottom: `1px solid ${C.line}` }}>
        ...
        {/* connection status — reflects whether live data actually loaded */}
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: liveOK ? C.green : C.amber, marginTop: 8, overflowX: "auto", scrollbarWidth: "none" }}>
          <span style={{ width: 7, height: 7, borderRadius: 9, background: liveOK ? C.green : C.amber, boxShadow: `0 0 8px ${liveOK ? C.green : C.amber}`, flexShrink: 0 }} />
          <span style={{ fontFamily: "ui-monospace", whiteSpace: "nowrap" }}>{exchange} · {liveOK ? t.live_real : t.live_sim}</span>
        </div>
...
function Dashboard({ t, lang, isMobile, active, setActive, tf, setTf, coin, price, chg, chartData, rsiVal, high, low, macdCross, buyPct, whaleEvents, netFlow, dataRef, alerts, setAlerts, scrollRow, exMul, exchange, setExchange, mtfRows, mtfVerdict, mtfBulls, mtfBears, watchSyms, setWatchSyms }) {
  ...
  // --- Fear & Greed: real data from alternative.me (free, no key, CORS-ok), fallback to derived ---
  const [realFng, setRealFng] = useState(null); // { now, yesterday, week } or null
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      try {
        ...
        const j = await r.json();
        const d = j && j.data;
        if (!d || !d.length || cancelled) return;
        setRealFng({
          now: parseInt(d[0].value, 10),
          yesterday: d[1] ? parseInt(d[1].value, 10) : null,
          week: d[7] ? parseInt(d[7].value, 10) : null,
        });
      } catch (e) { if (!cancelled) setRealFng(null); }
    };
    load();
    const id = setInterval(load, 600000); // refresh every 10 min (index only updates daily)
    return () => { cancelled = true; clearInterval(id); };
  }, []);
  ...
  // real value if available, else derived from market breadth
  const fngLive = !!(realFng && isFinite(realFng.now));
  const fng = fngLive ? realFng.now : Math.max(0, Math.min(100, Math.round(50 + avgChg * 6 + (buyPct - 50) * 0.4)));
  ...
  const fngHist = [
    { label: t.fng_yesterday, v: fngLive && realFng.yesterday != null ? realFng.yesterday : Math.max(0, Math.min(100, fng + Math.round((Math.sin(fng) * 8)))) },
    { label: t.fng_week, v: fngLive && realFng.week != null ? realFng.week : Math.max(0, Math.min(100, fng - Math.round((Math.cos(fng) * 12)))) },
  ];
  ...
  const FngPanel = (
    <Panel title={t.fng_title} icon={<Activity size={15} color={C.amber} />}
      right={<span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20, color: fngLive ? C.green : C.dim, border: `1px solid ${fngLive ? C.green : C.line}` }}>{fngLive ? t.fut_live : t.fut_derived}</span>}>
...
function TechnicalAnalysis({ t, lang, isMobile, coin, series, price, buyPct }) {
  ...
  const runAI = async () => {
    ...
    const sysLang = lang === "zh"
      ? `你是一位有十年實戰經驗的加密貨幣交易分析師，曾在交易所擔任首席分析師。根據提供的即時指標數據，用繁體中文寫一份專業的盤勢解讀，約 250-350 字，分成以下結構：

【方向判斷】開頭用一句話明確點出目前偏多、偏空還是盤整，並說明主要依據。
【訊號分析】挑出 2-3 個最關鍵的指標（例如 RSI、MACD、均線排列、布林通道、市場結構），解釋它們現在說明什麼，以及彼此是否互相印證或背離。用專業術語但要解釋清楚。
【操作建議】給出具體可執行的建議：理想進場區間、停損位置、目標價位，以及該用什麼倉位心態（重倉/輕倉/觀望）。
【風險提醒】點出目前盤勢最大的風險或變數。

語氣要像專業分析師寫研報，有條理、有depth、有信心，但保持客觀。可以適度使用換行讓結構清晰。結尾簡短提醒這是分析參考、非投資建議。`
      : `You are a crypto trading analyst with 10 years of hands-on experience, formerly a head analyst at an exchange. Based on the live indicator data, write a professional market analysis in English, about 250-350 words, structured as follows:

[Direction] Open with one clear sentence on whether the market is bullish, bearish, or ranging, and the main basis.
[Signal Analysis] Pick the 2-3 most critical indicators (e.g. RSI, MACD, MA alignment, Bollinger, market structure), explain what they currently signal and whether they confirm or diverge from each other. Use professional terminology but explain clearly.
[Trade Setup] Give concrete, actionable guidance: ideal entry zone, stop-loss level, target prices, and what position mindset to take (heavy/light/wait).
[Risk] Flag the biggest risk or variable in the current setup.

Write like a professional analyst's research note — structured, with depth and confidence, yet objective. Use line breaks for clarity. End with a brief note that this is analysis, not financial advice.`;
    try {
      const { text, error } = await callAI(`${sysLang}\n\nData:\n${JSON.stringify(snapshot, null, 2)}`);
      if (text) setAiText(text); else setAiErr(error || t.ai_error);
    } catch (e) {
      setAiErr(String(e));
    } finally {
      setAiLoading(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 12, maxWidth: 920, margin: "0 auto" }}>
      ...
      <Panel title={`${coin.sym} · ${t.ai_title}`} icon={<Zap size={15} color={C.amber} />}
        ...
        {aiErr && (
          <div style={{ padding: 11, background: C.red + "1a", border: `1px solid ${C.red}55`, borderRadius: 8, color: C.red, fontSize: 12, marginBottom: 12, wordBreak: "break-word" }}>
            ⚠ {aiErr === true ? t.ai_error : aiErr}
          </div>
        )}
...
function FuturesData({ t, lang, isMobile, coin, price, series, buyPct }) {
  ...
  const [aiLoading, setAiLoading] = useState(false);
  const [aiErr, setAiErr] = useState("");
  const [liveFut, setLiveFut] = useState(null); // {fundingRate, holdVol, riseFallRate} from MEXC

  // fetch real MEXC contract data (funding + open interest) for crypto
  useEffect(() => {
    if (coin.cat === "commodity") { setLiveFut(null); return; }
    let cancelled = false;
    const load = async () => {
      try {
        const r = await fetch("/api/mexc?path=futticker&symbol=" + coin.sym + "_USDT");
        if (!r.ok) throw new Error("futticker " + r.status);
        const j = await r.json();
        const d = j && j.data ? j.data : null;
        if (!d || cancelled) return;
        setLiveFut({
          fundingRate: parseFloat(d.fundingRate),
          holdVol: parseFloat(d.holdVol),
          lastPrice: parseFloat(d.lastPrice),
          riseFallRate: parseFloat(d.riseFallRate),
        });
      } catch (e) { if (!cancelled) setLiveFut(null); }
    };
    load();
  ...
  const hasLive = !!(liveFut && isFinite(liveFut.fundingRate));

  // funding rate: REAL from MEXC if available, else derived. MEXC returns decimal (0.0001 = 0.01%)
  const fundingPct = hasLive ? liveFut.fundingRate * 100 : (mom * 0.18 + (buyPct - 50) * 0.0009);
  ...
  // open interest: REAL holdVol (contracts) × price if available, else derived
  const oiBase = (coin.base > 1000 ? 8.5e9 : coin.base > 10 ? 1.8e9 : 4.2e8);
  const oi = hasLive && isFinite(liveFut.holdVol) ? liveFut.holdVol * price : oiBase * (1 + mom * 2);
  const oiChange = hasLive && isFinite(liveFut.riseFallRate) ? liveFut.riseFallRate * 100 : mom * 100 * 1.4;
  const oiState = oiChange > 0.4 ? "rising" : oiChange < -0.4 ? "falling" : "div";

  // long/short ratio (MEXC public API doesn't expose this — derived from price action + buy pressure)
  ...
  const runFutAI = async () => {
    ...
    const snapshot = {
      ...
      futures: {
        dataSource: hasLive ? "funding rate and open interest are REAL live data from MEXC" : "funding/OI are estimated (live feed unavailable)",
    ...
    const sys = lang === "zh"
      ? `你是一位專精合約交易的加密貨幣分析師，有豐富的衍生品實戰經驗。根據提供的合約數據（資金費率、未平倉量、多空比、清算價位）與技術面（斐波那契、市場結構、RSI、MACD、均線），用繁體中文寫一份專業的合約盤勢解讀，約 280-380 字，分成以下結構：

【多空研判】明確判斷目前偏多還偏空，並說明哪些訊號互相共振（例如斐波支撐與多單清算區重疊、或資金費率與多空比的訊號一致）。
【人群與情緒】分析資金費率與多空比代表的市場情緒——是否過熱、擁擠、有軋空或插針的風險。這是合約最關鍵的反指標思維。
【清算結構】解讀清算地圖，指出價格上下方的清算密集區，以及主力可能掃哪一邊的流動性。
【操作策略】給出具體合約操作：方向、進場區間、停損（務必在爆倉價之前）、目標價、以及建議槓桿倍數與倉位控制。
【風險警示】點出這筆合約最大的風險。

語氣專業、有條理、像在寫合約交易研報。可用換行讓結構清晰。結尾提醒槓桿會放大虧損、務必設停損、本內容非投資建議。`
      : `You are a crypto analyst specializing in derivatives trading with deep hands-on futures experience. Based on the futures data (funding rate, open interest, long/short ratio, liquidation levels) and technicals (Fibonacci, market structure, RSI, MACD, MA), write a professional futures analysis in English, about 280-380 words, structured as:

[Bull/Bear Read] Clearly judge bullish vs bearish, explaining which signals confluence (e.g. Fib support overlapping a long-liquidation cluster, or funding aligning with the long/short ratio).
[Crowd & Sentiment] Analyze what funding rate and long/short ratio reveal about sentiment — overheated, crowded, squeeze or wick risk. This contrarian thinking is key in futures.
[Liquidation Structure] Read the liquidation map, point out clusters above and below price, and which side of liquidity big players may hunt.
[Strategy] Give concrete futures guidance: direction, entry zone, stop (must be before liquidation), targets, plus suggested leverage and position sizing.
[Risk] Flag the single biggest risk in this trade.

Professional, structured, like a futures research note. Use line breaks for clarity. End by reminding that leverage amplifies losses, always use a stop, and this is not financial advice.`;
    try {
      const { text, error } = await callAI(`${sys}\n\nData:\n${JSON.stringify(snapshot, null, 2)}`);
      if (text) setAiText(text); else setAiErr(error || t.fai_err);
    } catch (e) { setAiErr(String(e)); } finally { setAiLoading(false); }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 12, maxWidth: 920, margin: "0 auto" }}>
      ...
      <Panel title={`${coin.sym} · ${t.fai_title}`} icon={<Zap size={15} color={C.amber} />}
        ...
        {aiErr && (
          <div style={{ padding: 11, background: C.red + "1a", border: `1px solid ${C.red}55`, borderRadius: 8, color: C.red, fontSize: 12, marginBottom: 12, wordBreak: "break-word" }}>⚠ {aiErr === true ? t.fai_err : aiErr}</div>
        )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
        ...
        <Panel title={`${coin.sym} · ${t.fr_title}`} icon={<Zap size={15} color={C.amber} />}
          right={<span style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20, color: hasLive ? C.green : C.dim, border: `1px solid ${hasLive ? C.green : C.line}` }}>{hasLive ? t.fut_live : t.fut_derived}</span>
            <span style={{ fontSize: 10, color: C.dim }}>{t.fr_sub}</span>
          </span>}>
        ...
        <Panel title={`${coin.sym} · ${t.oi_title}`} icon={<Layers size={15} color={C.amber} />}
          right={<span style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20, color: hasLive ? C.green : C.dim, border: `1px solid ${hasLive ? C.green : C.line}` }}>{hasLive ? t.fut_live : t.fut_derived}</span>
            <span style={{ fontSize: 10, color: C.dim }}>{t.oi_sub}</span>
          </span>}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
